...
var initialColors = colorbrewer.Greens[6];
...
var attNames = ["Total", "Compete", "Connect", "Go_Wild", "Learn", "Move", "Play", "Splash", "Reflect", "Socio_Eco"];

var pcpdata = [];
var expressed=attNames[0];
var regions;
...
function setMap(){
  ...
  var map = new L.map('map').setView([41.65, -91.52], 12)
...
d3.json("data/WGS_Joined_Districts.json", function(error, jsonData) {
...
var recolorMap = colorScale(jsonData.features);
  ...
  function createDropdown(jsonData){
    ...
    .append("div")
    .html("<h4>Select Park Character: </h4>")
    .append("select")
  ...
  function highlight(data){// Variables
  var props = data.properties;
  console.log(key);
  var labelAttribute = "<h1>"+ roundRight(props[expressed]) + "</h1><br><b>Acres/1000: " + expressed + "</b>:" + props[key];
  var labelName = data.id;
  // Append label
  var infolabel = d3.select("#info-label")
    .append("div") //create the label div
    .attr("class", "infolabel")
    .attr("id", props[key]+"label")
  ...
  function dehighlight(data){console.log(data);
	var el = d3.select(this)
  .transition()
  .duration(100)
  ...
  function moveLabel() {
    var x = d3.event.clientX-3; //horizontal label coordinate based mouse position stored in d3.event
    var y = d3.event.clientY-3; //vertical label coordinate
